...
|TrxInitiation|	Support below value: `Customer`,`Merchant`,`NotSet`, if pass other value, then PF will use the default value set in PayFabric settings|
|TrxSchedule|	Support below value: `NotSet`,`Scheduled`,`ScheduledInstallment`,`ScheduledRecurring`,`Unscheduled`, if pass other value, then PF will use the default value set in PayFabric settings|
|AuthorizationType|	Support below value: `Incremental`,`Reauthorization`,`Resubmission`,`NotSet`, if pass other value, then PF will use the default value set in PayFabric settings|
